...
class TestAbsLoss(unittest.TestCase):
    ...
    def test_absloss_invalid_n(self):
        ...
        # load address of `array1` into register a1
        t.input_array("a1", arr1)
        # set a2 to an invalid length of the array (e.g. 0)
        t.input_scalar("a2", 0)
        # create a result array in the data section (fill values with -1)
        arr2 = t.array([-1] * len(arr0))
        # load address of `array2` into register a3
        ...
        # call the `abs_loss` function
        t.call("abs_loss")
        # check that the program terminated with exit code 36
        t.execute(code=36)
...
class TestSquaredLoss(unittest.TestCase):
    def test_simple(self):
        ...
        arr0 = t.array([1, 3, 7, 17])
        arr1 = t.array([4, 8, 2, 11])
    ...
    def test_squaredloss_invalid_n(self):
        ...
        t.input_array("a0", arr0)
        t.input_array("a1", arr1)
        # load invalid array length into argument register (e.g. 0)
        t.input_scalar("a2", 0)
        # create a result array in the data section (fill values with -1)
        arr2 = t.array([-1] * len(arr0))
        # load result array address into argument register
...
class TestInitializeZero(unittest.TestCase):
    ...
    def test_initialize_zero_invalid_n(self):
        ...
        # input an invalid array length (e.g., 0)
        t.input_scalar("a0", 0)
        # call the `initialize_zero` function
        t.call("initialize_zero")
        # check that the program terminated with exit code 36
